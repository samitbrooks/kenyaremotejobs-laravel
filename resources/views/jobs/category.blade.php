...
>
    ...
    <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        ...
        {{-- Category Hero --}}
        <x-reveal class="rounded-3xl border border-horizon-200/80 bg-gradient-to-br from-horizon-50/70 via-white to-orange-50/40 p-6 sm:p-10 shadow-xs">
            <div class="max-w-3xl">
                ...
                <h1 class="mt-4 text-3xl font-bold tracking-tight text-horizon-900 sm:text-4xl lg:text-5xl">
                    {{ $category['h1'] }}
                </h1>

                <p class="mt-4 text-base leading-relaxed text-foreground/70 sm:text-lg">
                    {{ $category['intro'] }}
                </p>

                <div class="mt-6 flex flex-wrap items-center gap-4 text-sm">
                    <div class="rounded-xl border border-black/5 bg-white px-4 py-2.5 shadow-xs">
                        <span class="block text-xs font-medium text-foreground/50">Typical Compensation</span>
                        <strong class="font-bold text-emerald-700">{{ $category['salary_range'] }}</strong>
                    </div>

                    <div class="rounded-xl border border-black/5 bg-white px-4 py-2.5 shadow-xs">
                        <span class="block text-xs font-medium text-foreground/50">Open Positions</span>
                        <strong class="font-bold text-horizon-800">{{ $totalJobs }} Active Listings</strong>
                    </div>

                    <div class="rounded-xl border border-black/5 bg-white px-4 py-2.5 shadow-xs">
                        <span class="block text-xs font-medium text-foreground/50">Location Reality</span>
                        <strong class="font-bold text-horizon-800">No US/EU Visa Needed</strong>
                    </div>
                </div>
            </div>
        </x-reveal>
        ...
        <section class="mt-12">
            <div class="flex flex-wrap items-center justify-between gap-4 border-b border-black/5 pb-4">
                ...
                <a href="{{ url('/jobs') }}" class="text-sm font-semibold text-sunrise-600 hover:underline">
                    View all 800+ jobs &rarr;
                </a>
            </div>

            @if ($jobs->isEmpty())
                <p class="mt-8 rounded-2xl bg-horizon-50 p-8 text-center text-foreground/60">
        ...
        @if (! empty($category['faqs']))
            <section class="mt-16 rounded-3xl border border-black/5 bg-white p-6 sm:p-10 shadow-sm">
                <div class="max-w-3xl">
                    <span class="text-xs font-bold uppercase tracking-wider text-sunrise-600">Got Questions?</span>
                    <h2 class="mt-1 text-2xl font-bold sm:text-3xl">Frequently Asked Questions</h2>
                    ...
                    <div class="mt-8 space-y-6">
                        @foreach ($category['faqs'] as $faq)
                            <div class="border-b border-black/5 pb-6 last:border-b-0 last:pb-0">
        ...
        <section class="mt-14">
            <h2 class="text-xl font-bold text-foreground">Explore Other Remote Roles in Kenya</h2>
            <p class="mt-1 text-sm text-foreground/60">Browse other high-demand remote job categories hiring East African talent.</p>

            <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($allCategories as $cat)
                    <a
                        href="{{ url('/remote-jobs/'.$cat['slug']) }}"
                        class="group rounded-2xl border border-black/5 bg-white p-4 shadow-xs transition duration-200 hover:-translate-y-0.5 hover:border-horizon-300 hover:shadow-md"
                    >
                        <span class="text-xs font-semibold text-sunrise-600">{{ $cat['badge'] }}</span>
                        <h3 class="mt-1 text-sm font-bold text-foreground group-hover:text-horizon-800">
                            {{ $cat['name'] }} Jobs
                        </h3>
                        <p class="mt-1 text-xs text-foreground/50 line-clamp-2">
