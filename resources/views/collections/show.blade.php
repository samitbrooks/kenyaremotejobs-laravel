...
>
    ...
    <div class="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        ...
        {{-- Hero Header --}}
        <x-reveal class="rounded-3xl border border-black/5 bg-gradient-to-br from-white via-white to-horizon-50/50 p-8 shadow-sm sm:p-12">
            <div class="max-w-3xl">
                <div class="flex flex-wrap items-center gap-2.5">
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-sunrise-100 border border-sunrise-200 px-3 py-0.5 text-xs font-bold text-sunrise-900">
                        <x-icon :name="$collection['icon']" class="h-3.5 w-3.5 text-sunrise-600" />
                        {{ $collection['badge'] }}
                    </span>
                    <span class="inline-flex items-center gap-1 rounded-full bg-emerald-100 border border-emerald-200 px-3 py-0.5 text-xs font-semibold text-emerald-800">
                ...
                <div class="mt-6 flex flex-wrap items-center gap-4 text-xs text-foreground/60">
                    <div class="flex items-center gap-1.5">
                        <x-icon name="check" class="h-4 w-4 text-emerald-600" />
                        <span>Curated for East Africa Time (UTC+3)</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <x-icon name="coin" class="h-4 w-4 text-sunrise-600" />
                        <span>USD &amp; Wise Payouts</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <x-icon name="globe" class="h-4 w-4 text-horizon-600" />
        ...
        <div class="mt-12">
            <div class="flex items-center justify-between border-b border-black/5 pb-4">
                <div>
                    <h2 class="text-2xl font-bold text-foreground">Available Remote Roles ({{ $jobs->count() }})</h2>
                    <p class="text-xs text-foreground/60 mt-0.5">Updated in real-time from vetted international employers</p>
                </div>
        ...
        @if (! empty($collection['faqs']))
            <div class="mt-16 rounded-3xl border border-black/5 bg-white p-8 shadow-sm sm:p-10">
                <div class="max-w-2xl">
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-horizon-100 px-3 py-1 text-xs font-bold text-horizon-900">
                ...
                <div class="mt-8 space-y-4">
                    @foreach ($collection['faqs'] as $faq)
                        <div class="rounded-2xl border border-black/5 bg-horizon-50/40 p-5">
                            <h4 class="font-bold text-base text-horizon-950 flex items-start gap-2.5">
                                <span class="text-sunrise-500 font-extrabold">Q:</span>
                                <span>{{ $faq['question'] }}</span>
                            </h4>
                            <p class="mt-2 text-sm text-foreground/75 leading-relaxed pl-6">
                                {{ $faq['answer'] }}
                            </p>
                        </div>
                    @endforeach
        ...
        @if (! empty($otherCollections))
            <div class="mt-16 border-t border-black/5 pt-10">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-xl font-bold text-foreground">Explore Other Curated Collections</h3>
                        <p class="text-xs text-foreground/50">Find remote jobs matched to your work style</p>
                    </div>
                    <a href="{{ url('/collections') }}" class="text-xs font-semibold text-sunrise-600 hover:underline">
                ...
                <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ($otherCollections as $other)
                        <a
                            ...
                            class="group rounded-2xl border border-black/5 bg-white p-5 shadow-xs transition hover:-translate-y-1 hover:border-horizon-300 hover:shadow-md"
                        >
                            <span class="inline-block rounded-full bg-horizon-100 px-2.5 py-0.5 text-[10px] font-semibold text-horizon-800">
                                {{ $other['badge'] }}
                            </span>
                            <p class="font-bold text-sm text-foreground mt-2 group-hover:text-sunrise-600 transition-colors leading-snug">
                                {{ $other['title'] }}
                            </p>
                        </a>
                    @endforeach
