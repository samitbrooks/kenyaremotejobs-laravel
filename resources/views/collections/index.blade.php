...
>
    <div class="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        ...
        <x-reveal class="text-center max-w-3xl mx-auto">
            <span class="inline-flex items-center gap-1.5 rounded-full bg-teal-50 border border-teal-200 px-3.5 py-1 text-xs font-bold text-teal-800">
            ...
            @foreach ($collections as $i => $collection)
                <x-reveal :delay="min($i, 5) * 60" class="h-full">
                    <a
                        ...
                        class="group flex h-full flex-col justify-between rounded-2xl border border-slate-200 bg-white p-7 shadow-2xs transition-all duration-200 hover:-translate-y-1 hover:border-teal-500 hover:shadow-md"
                    >
                        <div>
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex h-12 w-12 items-center justify-center rounded-xl border border-slate-200 bg-slate-100 text-teal-700 shadow-2xs group-hover:bg-teal-50 group-hover:text-teal-700 transition-colors">
                                    <x-icon :name="$collection['icon']" class="h-6 w-6" />
                                </div>
                                <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 border border-slate-200 px-2.5 py-0.5 text-[11px] font-semibold text-slate-800">
                                    {{ $collection['badge'] }}
                                </span>
                            </div>

                            <h2 class="mt-5 font-bold text-xl text-slate-900 group-hover:text-teal-700 transition-colors leading-snug">
                                {{ $collection['title'] }}
                            </h2>
                            ...
                            @if (! empty($collection['faqs']))
                                <div class="mt-4 rounded-xl bg-slate-50 p-3 text-[11px] text-slate-600 border border-slate-200">
                                    <span class="font-semibold text-slate-900">FAQ:</span> {{ $collection['faqs'][0]['question'] }}
                                </div>
                            @endif
                        </div>
